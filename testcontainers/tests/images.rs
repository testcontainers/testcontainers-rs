...
fn mysql_one_plus_one() {
    ...
    let mut conn = mysql::Conn::new(connection_string).unwrap();

    let rows: Vec<Row> = conn.query("SELECT 1 + 1").unwrap();
    assert_eq!(rows.len(), 1);

    let first_row = &rows[0];
    let first_column: i32 = first_row.get(0).unwrap();
    assert_eq!(first_column, 2);
}
...
fn mysql_one_plus_one_with_custom_mapped_port() {
    ...
    let connection_string = &format!("mysql://root:password@127.0.0.1:{}/test", free_local_port);
    ...
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].get::<i32, &str>("result").unwrap(), 2);
}
