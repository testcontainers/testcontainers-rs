use crate::core::{
    self, env, env::GetEnvValue, Container, DockerOps, DockerRun, Image, Logs, RunArgs,
};
use std::{
    collections::HashMap,
    ffi::{OsStr, OsString},
...
impl DockerRun for Cli {
    fn run_with_args<I: Image>(&self, image: I, run_args: RunArgs) -> Container<'_, I> {
        ...
        Container::new(container_id, &self.inner, image, self.inner.command)
    }
}

impl DockerOps for Client {
    ...
    fn drop(&mut self) {
        ...
        match self.inner.command {
            env::Command::Remove if created_networks => {
                self.inner.delete_networks(networks.iter());
            }
            env::Command::Remove => {
                // nothing to do
            }
...
#[cfg(test)]
mod tests {
    use crate::{core::WaitFor, images::generic::GenericImage, DockerRun, Image};
  ...
  "NetworkSettings": {
    ...
    fn cli_rm_command_should_panic_on_invalid_container() {
        ...
        docker.inner.rm("!INVALID_NAME_DUE_TO_SYMBOLS!");
        unreachable!()
    }
